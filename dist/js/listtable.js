 ...
 * 翻页
 */
listTable.gotoPage = function(page, listType)
{
  ...
  this.filter['page_size'] = this.getPageSize();
	//alert(page + '|' +  this.filter['page_size']);
 ...
 * 载入列表
 */
listTable.loadList = function(listType)
{
	//alert('in  list table ');
	var isForeign = document.getElementById('is_foreign_goods').value;
  var args = "&act="+this.query+"&list_type=" +listType + "&is_foreign=" + isForeign +  "" + this.compileFilter();
	//alert( this.url + args);
  Ajax.call(this.url, args, this.listCallback, "POST", "JSON");
}
...
listTable.gotoPageFirst = function(listType)
{
  if (this.filter.page > 1)
  {
    listTable.gotoPage(1,listType);
  }
}

listTable.gotoPagePrev = function(listType)
{
	//alert(listType + 'page' + this.filter.page);
  if (this.filter.page > 1)
  {
    listTable.gotoPage(this.filter.page - 1,listType);
  }
}

listTable.gotoPageNext = function(listType)
{
	//var listType =  document.getElementById("list_type_value").value;
	//alert(listType + '|' + this.filter.page + '|' + listTable.pageCount);
  if (this.filter.page < listTable.pageCount)
  {
    listTable.gotoPage(parseInt(this.filter.page) + 1, listType );
  }
}

listTable.gotoPageLast = function(listType)
{
  if (this.filter.page < listTable.pageCount)
  {
    listTable.gotoPage(listTable.pageCount,listType);
  }
}
...
listTable.getPageSize = function()
{
  ...
  if (pageSize)
  {
    ...
    document.cookie = "ZYSCP[page_size]=" + ps + ";";
  }
